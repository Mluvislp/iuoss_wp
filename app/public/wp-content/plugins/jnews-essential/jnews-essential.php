...
defined( 'JNEWS_ESSENTIAL_VERSION' ) or define( 'JNEWS_ESSENTIAL_VERSION', '11.6.5' );
...
add_filter(
	...
	function( $plugin_list ){
		$plugin_list[] = 'JNews - Essential';
		return $plugin_list;
	}
);

add_action( 'admin_notices', 'jnews_essential_plugin_notice' );

if ( ! function_exists( 'jnews_essential_plugin_notice' ) ) {
	/**
	 * JNews Bookmark plugin notice
	 */
	function jnews_essential_plugin_notice() {
			...
			if( ! defined( 'JNEWS_11_6_0_PLUGIN_COMPAT' ) ) {
				...
				echo wp_kses(
					'<div class="notice notice-error">
						<p>
							<span class="jnews-notice-heading">' . __( 'JNews Plugins Compatibility', 'jnews-essential' ) . '</span>
							<span>' . __( 'These plugins below only compatible with the <strong>JNews Theme version 11.6.0</strong> and above. Your current JNews version is ' . JNEWS_THEME_VERSION . ', please update JNews theme to the latest version to ensure proper functionality and compatibility.', 'jnews-essential' ) . '</span><ul>' .
								$plugins .
							'</ul>
							<span class="jnews-notice-button">
								<a href="' . wp_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( JNEWS_THEME_TEXTDOMAIN ) ), 'upgrade-theme_' . JNEWS_THEME_TEXTDOMAIN ) . '" class="button-primary">' . __( 'Update Theme Now', 'jnews-essential' ) . '</a>
							</span>
						</p>
					</div>',
					array(
