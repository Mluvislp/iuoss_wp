...
final class Font {
	...
	/**
	 * Determine if a font-name is a custom font or not and return the custom font value.
	 *
	 * @static
	 * @access public
	 *
	 * @param  string $font_name The name of the font we want to check.
	 * @param  array  $font_variant The variants of the font we want to check.
	 *
	 * @return bool|array
	 */
	public static function is_custom_font( $font_name, $font_variant ) {
				...
				if ( $font['font_name'] === $font_name ) {
					...
					if ( ! empty( $font_variant ) ) {
						foreach ( $font_variant as $variant ) {
							/* load only selected font variant*/
							if ( $custom_variant === $variant ) {
								...
								$custom_fonts_final[ $font['font_name'] . $custom_variant ] = $variants_src;
