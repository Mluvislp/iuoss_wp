...
class Style_Generator {
	...
	public function generate_css_file() {
		...
		if ( $this->check_folder() ) {
			if ( $wp_filesystem->put_contents( $file_path, $styles, 0777 ) ) {
				return true;
			} elseif ( file_put_contents( $file_path, $styles ) ) {
					return true;
			}
		}
	...
	public function get_google_font_setting() {
		...
		if ( $settings ) {
			foreach ( $settings as $setting ) {
				$option = get_theme_mod( $setting ); /* see 329ZM682 */
		...
		if ( is_customize_preview() ) {
			foreach ( $fonts as $font ) {
				if ( Font::is_google_font( $font['name'] ) && ! get_theme_mod( 'jnews_gdpr_google_font_disable', false ) ) {
					...
					$custom = Font::is_custom_font( $font['name'], $font['variant'] );
					if ( $custom && is_array( $custom ) ) {
						$custom_font_css = '';
						foreach ( $custom as $key => $custom_font ) {
							$custom_font_css .= $custom_font . ' ';
						}
						$custom_font_css = '<style id="' . $font['setting'] . '-css">' . $custom_font_css . '</style>';
						add_action(
							'wp_head',
							function () use ( $custom_font_css ) {
	...
	/**
	 * Add custom font css ro jeg_dynamic_css
	 *
	 * @param string $generated_style dynamic style css.
	 *
	 * @return string
	 */
	public function custom_font_css( $generated_style ) {
		if ( ! empty( $this->custom_fonts ) ) {
			return $this->custom_fonts . $generated_style;
		} else {
			$generated_style;
		}
	}
