...
class JNews_Social_Meta_Option {
	...
	private function __construct() {
		if ( class_exists( 'Jeg\Customizer\Customizer' ) ) {
			...
			$this->set_general_meta_section();
			$this->set_social_meta_section();
		}
	}

	public function set_general_meta_section() {
		$general_meta_section = array(
			'id'       => 'jnews_header_meta_section',
			'title'    => esc_html__( 'Meta Header Tags', 'jnews-meta-header' ),
			'panel'    => 'jnews_global_panel',
			'priority' => 252,
			'type'     => 'jnews-lazy-section',
		);

		$this->customizer->add_section( $general_meta_section );
	}

	public function set_social_meta_section() {
