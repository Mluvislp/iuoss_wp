...
class Tax_Meta_Class {
  ...
  public function show($term_id) {
    ...
    if ('image' != $field['type'] && $field['type'] != 'repeater')
        ...
        if($multiple && is_array($meta[0])) {
          $meta = is_array( $meta ) ? array_map( 'esc_attr', $meta[0] ) : esc_attr( $meta );
        } else {
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_select( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      echo "<select style='{$field['style']}' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple' class='at-select wordpress-helpdesk-select2 select2'" : "class='at-select'" ) . ">";
