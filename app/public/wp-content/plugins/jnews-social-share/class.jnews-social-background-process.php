...
class JNews_Social_Background_Process {
	...
	public function fetch_data( $post_id ) {
		...
		$socials_api = jnews_get_option( 'single_social_share_enable_api', array( 'facebook', 'pinterest', 'buffer', 'vk' ) );

		if ( ! empty( $socials ) ) {
			foreach ( $socials as $social ) {
				$social_share = $social['social_share'];
				if ( in_array( $social_share, $socials_api ) ) {
					$url = $this->get_api_url( $social_share );
					if ( ! empty( $url ) ) {
	...
	protected function get_api_url( $social ) {
		...
		switch ( $social ) {
			case 'facebook':
				...
				if ( ! empty( $fb_token ) ) {
					$api_url = 'https://graph.facebook.com/22/?id=' . $this->post_url() . '&fields=engagement&access_token=' . $fb_token; /* see tBj9C1xF */
				}
				break;
			case 'pinterest':
