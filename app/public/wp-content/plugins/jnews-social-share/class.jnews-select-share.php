	...
	class JNews_Select_Share {
		...
		public function load_asset() {
			if ( is_single() || is_page() ) {
				if ( is_singular( 'post' ) || ( ( 'default' === get_post_meta( get_the_ID(), '_wp_page_template', true ) || 'elementor_theme' === get_post_meta( get_the_ID(), '_wp_page_template', true ) ) && ( jnews_get_metabox_value( 'jnews_single_page.share_position', 'top' ) !== 'hide' ) ) ) {
		...
		public static function get_select_share_data( $social, $post_id ) {
			...
			$url       = function_exists( 'jnews_encode_url' ) ? jnews_encode_url( $post_id ) : '';
			$url       = apply_filters( 'jnews_get_permalink', $url );
			...
			switch ( $social ) {
				...
				case 'email':
					...
					break;
				case 'threads':
					$button_url = 'https://www.threads.net/intent/post?text=[selected_text]&url=[url]';
					$data       = array(
						'image_url'  => null,
						'title'      => $title,
						'post_url'   => $url,
					...
					break;
				case 'bluesky':
					$button_url = 'https://bsky.app/intent/compose?text=[selected_text]';
					$data       = array(
						'image_url'  => null,
						'title'      => $title,
						'post_url'   => $url,
		...
		protected function build_social_button( $social ) {
			...
			if ( isset( $button_data['svg_image'] ) ) {
				$icon   = function_exists( 'jnews_get_svg' ) ? jnews_get_svg( $button_data['svg_image'] ) : '';
