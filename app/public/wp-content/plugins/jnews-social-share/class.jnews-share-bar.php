...
class JNews_Share_Bar {
	...
	public function flat_share_output() {
		...
		foreach ( $buttons as $button ) {
			...
			if ( 'twitter' === $button['social_share'] ) { /*  see vxBj2MPD */
				$button_text .= "<a href=\"{$share_url}\" aria-label=\"" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucfirst( $button['social_share'] ) . '" class="jeg-icon"> ' . jnews_get_svg( 'twitter' ) . '</a>';
			} elseif ( 'line' === $button['social_share'] ) {
				$button_text .= "<a href=\"{$share_url}\" aria-label=\"" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucfirst( $button['social_share'] ) . '" class="jeg-icon">' . jnews_get_svg( 'line' ) . '</a>';
			} elseif ( 'threads' === $button['social_share'] ) {
				$button_text .= "<a href=\"{$share_url}\" aria-label=\"" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucfirst( $button['social_share'] ) . '" class="jeg-icon">' . jnews_get_svg( 'threads' ) . '</a>';
			} else {
				$button_text .= "<a href=\"{$share_url}\" aria-label=\"" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucfirst( $button['social_share'] ) . "\"><i class=\"fa {$icon_class}\"></i></a>";
			}
		}
	...
	protected function secondary_button() {
		...
		if ( ! empty( $shares ) ) {
			...
			$secondary_button =
                ...
                <a href=\"#\" class=\"jeg_btn-toggle\" aria-label=\"" . esc_html__( 'Share on another social media platform', 'jnews-social-share' ) . '"><i class="fa fa-share"></i></a>';
		}
	...
	public static function get_social_share_url( $social, $post_id ) {
		...
		switch ( $social ) {
			...
			case 'twitter':
				...
				break;
			case 'threads':
				$button_url = 'https://www.threads.net/intent/post?text=' . $title . '&url=' . $url;
				break;
			case 'xing':
				$button_url = 'https://www.xing.com/spi/shares/new?url=' . $url;
				break;
			case 'pinterest':
			...
			case 'qrcode':
			case 'wechat':
				$button_url = 'https://quickchart.io/qr?size=400&text=' . $url; //see LQIvV8No
				break;
			case 'line':
			...
			case 'weibo':
				...
				break;
			case 'bluesky':
				$share_text = mb_strlen( $title . '%20' . $url ) > 300 ? $url : $title . '%20' . $url;
				$button_url = 'https://bsky.app/intent/compose?text=' . $share_text;
				break;
			default:
				$button_url = $url;
				break;
	...
	protected function get_button_class( $social ) {
		switch ( $social ) {
			...
			case 'weibo':
				...
				break;
			case 'xing':
				$button_class = 'jeg_btn-xing';
				break;
			case 'threads':
				$button_class = 'jeg_btn-threads';
				break;
			case 'bluesky':
				$button_class = 'jeg_btn-bluesky';
				break;
			default:
				$button_class = '';
				break;
	...
	protected function build_button( $social, $text = '' ) {
		...
		if ( $social === 'twitter' ) {
			...
			$button = "<a href=\"{$button_url}\" rel='nofollow' aria-label='" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucwords( $social ) . "'{$additional_attribute} class=\"{$button_class} {$expanded}\"><i class=\"{$icon_class}\">" . $svg . "</i>$sharetext</a>";
		} elseif ( $social === 'line' ) {
			$svg    = jnews_get_svg( 'line' );
			$button = "<a href=\"{$button_url}\" rel='nofollow' aria-label='" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucwords( $social ) . "'{$additional_attribute} class=\"{$button_class} {$expanded}\"><i class=\"{$icon_class}\">" . $svg . "</i>$sharetext</a>";
		} elseif ( $social === 'threads' ) {
			$svg    = jnews_get_svg( 'threads' );
			$button = "<a href=\"{$button_url}\" rel='nofollow' aria-label='" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucwords( $social ) . "'{$additional_attribute} class=\"{$button_class} {$expanded}\"><i class=\"{$icon_class}\">" . $svg . "</i>$sharetext</a>";
		} elseif ( $social === 'bluesky' ) {
			$svg    = jnews_get_svg( 'bluesky' );
			$button = "<a href=\"{$button_url}\" rel='nofollow' aria-label='" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucwords( $social ) . "'{$additional_attribute} class=\"{$button_class} {$expanded}\"><i class=\"{$icon_class}\">" . $svg . "</i>$sharetext</a>";
		} else {
			$button = "<a href=\"{$button_url}\" rel='nofollow' aria-label='" . esc_html__( 'Share on ', 'jnews-social-share' ) . ucwords( $social ) . "'{$additional_attribute} class=\"{$button_class} {$expanded}\"><i class=\"{$icon_class}\"></i>$sharetext</a>";
		}
