...
class Captcha {
	...
	public function render_element( $type, $echo = true ) {
		if ( $this->can_render_script( $type ) ) {
			...
			?>
			<div class="g-recaptcha" data-sitekey="<?php echo static::$site_key; ?>" data-theme="<?php echo esc_attr( $dark_mode ); ?>"></div>
			<?php
			if ( ! $echo ) {
				return ob_get_clean();
			}
	 ...
	 *
	 * @param boolean $die do wp_die if inbalid.
	 * @param string  $type captcha location.
	 * @param boolean $skip skip recaptcha vertification.
	 * @return boolean
	 */
	public function check_recaptcha( $die = false, $type = '', $skip = false ) {
		$recaptcha = true;
		if ( ( isset( $_POST['g-recaptcha-response'] ) || $this->can_render_script( $type ) ) && ! $skip ) { /* see iPVPiprs */
