...
function jnews_get_share_position( $post_id ) {
	...
	if ( is_page( $post_id ) ) {
		$position = jnews_get_metabox_value( 'jnews_single_page.share_position', 'top', $post_id );
	}

	if ( jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id ) && jnews_get_metabox_value( 'jnews_single_post.override_template', false, $post_id ) ) {
		$position = jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id );
	}
...
function jnews_show_view_tag( $bool, $post_id ) {
	if ( ! is_page( $post_id ) ) {
		if ( ( jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id ) === 'top' || jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id ) === 'topbottom' ) && jnews_get_metabox_value( 'jnews_single_post.override_template', false, $post_id ) ) {
			if ( jnews_get_metabox_value( 'jnews_single_post.override.0.show_view_counter', false, $post_id ) ) {
				return true;
			} else {
				return false;
			}
...
function jnews_show_share_tag( $bool, $post_id ) {
	if ( ! is_page( $post_id ) ) {
		if ( ( jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id ) === 'top' || jnews_get_metabox_value( 'jnews_single_post.override.0.share_position', 'top', $post_id ) === 'topbottom' ) && jnews_get_metabox_value( 'jnews_single_post.override_template', false, $post_id ) ) {
			if ( jnews_get_metabox_value( 'jnews_single_post.override.0.show_share_counter', false, $post_id ) ) {
				return true;
			} else {
				return false;
			}
...
if ( ! function_exists( 'jnews_coment_custom_fields' ) ) {
	...
	function jnews_coment_custom_fields( $fields ) {
		...
		$fields['privacy_policy'] = '<p class="comment-form-privacy_policy">
										<label for="privacy_policy">' . jnews_get_translated_setting( get_theme_mod( 'jnews_gdpr_comment_label', esc_html__( 'Privacy Policy Agreement', 'jnews' ) ) ) . ' ' . ( $required_email ? '<span class="required">*</span>' : '' ) . '</label>
										<input id="privacy_policy" name="privacy_policy" ' . $required_field . " type='checkbox'> " . jnews_get_translated_setting( get_theme_mod( 'jnews_gdpr_comment_text', __( 'I agree to the Terms &amp; Conditions and <a href="#">Privacy Policy</a>.', 'jnews' ) ) ) . '
									</p>';
...
if ( ! function_exists( 'jnews_footer_cookie_law_policy' ) ) {
	...
	function jnews_footer_cookie_law_policy() {
		$output = '<div class="jnews-cookie-law-policy">' . jnews_get_translated_setting( get_theme_mod( 'jnews_gdpr_cookie_text', __( 'This website uses cookies. By continuing to use this website you are giving consent to cookies being used. Visit our <a href="#">Privacy and Cookie Policy</a>.', 'jnews' ) ) ) . ' <button data-expire="' . get_theme_mod( 'jnews_gdpr_cookie_expire', 7 ) . '" class="btn btn-cookie">' . jnews_get_translated_setting( get_theme_mod( 'jnews_gdpr_cookie_button', esc_html__( 'I Agree', 'jnews' ) ) ) . '</button></div>';
